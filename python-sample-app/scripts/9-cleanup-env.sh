...
delete_ssm_parameters() {
    ...
    # Delete fault injection parameter
    if aws ssm get-parameter --name "/python-sample-app/mysql/fault-injection" --region ${AWS_REGION} &>/dev/null; then
        log "DEBUG" "Deleting SSM parameter /python-sample-app/mysql/fault-injection..."
        aws ssm delete-parameter \
            --name "/python-sample-app/mysql/fault-injection" \
            --region ${AWS_REGION} || true
        log "SUCCESS" "SSM parameter /python-sample-app/mysql/fault-injection deleted"
    else
        log "WARNING" "SSM parameter /python-sample-app/mysql/fault-injection not found"
    fi
...
print_cleanup_summary() {
    log "SUCCESS" "\n======================================="
    log "SUCCESS" "Cleanup initiated successfully!"
    log "SUCCESS" "======================================="
    
    log "INFO" "\nAll AWS resources are being deleted"
    log "WARNING" "Check AWS CloudFormation Console to monitor deletion progress"
}
