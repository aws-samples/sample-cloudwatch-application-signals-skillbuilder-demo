...
restore_normal_config() {
    ...
    aws ssm put-parameter \
        ...
        --value "false" \
        --type "String" \
        --description "Database fault injection flag for python-sample-app (disabled)" \
        --overwrite \
        --region ${AWS_REGION}
...
show_restoration_summary() {
    ...
    log "INFO" "\nDatabase delays removed"
    log "INFO" "Application performance should return to normal"
}
...
parse_arguments() {
    if [ "$1" = "--help" ]; then
        ...
        log "INFO" "  3. Check traffic generator status (no changes made)"
