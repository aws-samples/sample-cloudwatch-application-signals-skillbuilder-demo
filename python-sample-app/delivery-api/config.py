...
class Config:
    ...
    def FAULT_INJECTION(self) -> bool:
        """Get fault injection flag from SSM Parameter Store."""
        value = self._get_ssm_parameter('/python-sample-app/mysql/fault-injection', 'false').lower()
        result = value in ('true', '1', 'yes', 'on')
        logging.debug(f"FAULT_INJECTION property returning: {result}")
        return result
