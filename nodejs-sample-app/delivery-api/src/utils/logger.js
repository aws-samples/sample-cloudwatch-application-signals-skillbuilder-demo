...
const cloudWatchFormatter = winston.format.combine(
  ...
  winston.format.printf((info) => {
    const logEntry = {
      ...
      service: info.service || 'delivery-api',
...
const developmentFormatter = winston.format.combine(
  ...
  winston.format.printf((info) => {
    ...
    return `${timestamp} ${level} [${service || 'delivery-api'}]${corrId}: ${message}${metaStr}`;
  })
);
...
logger.logDatabase = function(operation, duration, additionalMeta = {}) {
  const logLevel = duration > 5000 ? 'warn' : duration > 1000 ? 'info' : 'debug';
  const performanceCategory = duration > 5000 ? 'slow' : duration > 1000 ? 'normal' : 'fast';
  
  this[logLevel]('Database operation', {
    database: true,
    operation,
