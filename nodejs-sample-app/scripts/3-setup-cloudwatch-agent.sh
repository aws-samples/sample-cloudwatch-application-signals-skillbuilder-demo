...
start_application_signals() {
    log INFO "Starting Application Signals discovery..."
    if aws application-signals start-discovery --region "$AWS_REGION" 2>/dev/null; then
        log SUCCESS "Application Signals discovery started successfully"
    else
        log WARNING "Failed to start Application Signals discovery. This might affect signals collection."
    fi
}

setup_cloudwatch_service_account() {
    log INFO "Setting up CloudWatch service account for Node.js applications..."
...
setup_cloudwatch_addon() {
    log INFO "Setting up Amazon CloudWatch Observability EKS add-on for Node.js services..."
...
main() {
    log INFO "Starting CloudWatch setup for Node.js applications"
    read_cluster_config
    verify_cluster_context
    setup_service_linked_role
    start_application_signals
    ...
    log SUCCESS "\n========================================="
    log SUCCESS "CloudWatch setup for Node.js apps completed!"
    log SUCCESS "========================================="
    ...
    log INFO "\nFeatures enabled:\n- 1.Enhanced Container Insights\n- 2.CloudWatch Application Signals for Node.js\n- 3.Container logs via Fluent Bit\n- 4.Node.js auto-instrumentation support"
    log INFO "\nNext Steps:\n- Run 4-annotate-workloads.sh to enable auto-instrumentation\n- Deploy your Node.js applications if not already deployed"
}
